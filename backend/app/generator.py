...
def design(
    target, num_molecules: int = 12, reference_smiles: Optional[str] = None
) -> list[dict]:
    """Return deduped candidates [{smiles, inchikey, design_run_id}].

    `reference_smiles` seeds MolMIM optimization (GENERATOR_ENGINE=molmim); the
    default BoltzMol generator does not take a reference and ignores it.
    """
    engine = os.environ.get("GENERATOR_ENGINE", "boltz").strip().lower()
    if engine == "molmim":
        raw = [
            {"smiles": m["smiles"], "design_run_id": "molmim"}
            for m in get_nim_client().generate(target, num_molecules, reference_smiles)
        ]
    else:
